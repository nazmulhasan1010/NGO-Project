...
function imagePreView(file) {
    ...
    reader.onload = function (event) {
        ...
        $('#restoreImage').css('display', 'flex');
        $('#old_image').val('change');
        $('.imagePreViewSelect').removeClass('imagePreViewEmpty').addClass('imagePreViewModal');
    }
}

$('#restoreImage').click(function () {
    let imageLink = $(this).data('id');
    $('#imagePreView').attr('src', window.location.origin + "/storage/" + imageLink);
    $('#old_image').val(imageLink);
});
// image preview end
...
$('#videoLink').keyup(function () {
    let url = $(this).val();
    let img = window.location.origin + '/assets/backend/images/avatar/film.png';
    let preview = "<div class='preview-img'> <img src='" + img + "'" +
        " class='imagePreView imagePreViewSelect imagePreViewEmpty'" +
        "</div>";
    let previewModal = '.preview-img';
    videoPreView(url, preview,previewModal);

})
$('#editVideoLink').keyup(function () {
    let url = $(this).val();
    let preview = $('#old_video').val();
    let previewModal = '.videoEditPreview';
    videoPreView(url, preview, previewModal);
})

function videoPreView(url, preview, previewModal) {
    if (url !== '') {
        if (url.match(/(?:youtu\.be\/|youtube\.com(?:\/embed\/|\/v\/|\/watch\?v=|\/user\/\S+|\/ytscreeningroom\?v=))([\w\-]{10,12})\b/)) {
            $( previewModal).html(url);
        } else {
            $( previewModal).html(preview);
        }
    } else {
        $(previewModal).html(preview);
    }
